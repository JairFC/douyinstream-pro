...
class HistoryCard(ctk.CTkFrame):
    ...
    def _toggle_favorite(self) -> None:
        ...
        # Check if widget still exists before updating to prevent TclError
        try:
            if self._fav_btn.winfo_exists():
    ...
    def set_live_status(self, status: str) -> None:
        ...
        Args:
            status: "live", "offline", "checking", or "unknown"
        """
        try:
            if not self._status_indicator.winfo_exists():
                return
            ...
            if status == "live":
                ...
                self._status_indicator.configure(
                    text="⚪",
                    text_color="#888888"  # Light gray
                )
        except Exception as e:
            logging.debug(f"[HistoryCard] Error updating status: {e}")
