...
class DouyinStreamApp(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self._checking_favorites = False  # Flag for favorites check in progress
        ...
        if geometry:
            ...
            try:
                ...
                if match:
                    w = int(match.group(1))
                    h = int(match.group(2))
                    x_sign = 1 if match.group(3) == '+' else -1
                    x = x_sign * int(match.group(4))
                    y_sign = 1 if match.group(5) == '+' else -1
                    y = y_sign * int(match.group(6))
                    
                    # Validate position is visible on a connected monitor
                    from core.monitor_utils import is_position_visible
                    if is_position_visible(x, y, w, h):
                        use_saved_geometry = True
                    else:
                        print(f"[App] Saved geometry {geometry} is off-screen, will center on active monitor")
            except Exception as e:
                print(f"[App] Error parsing geometry '{geometry}': {e}")
        
        if not use_saved_geometry:
            # Default to centering on the monitor where the mouse is
            try:
                from core.monitor_utils import get_monitor_at_point
                # Get mouse position
                x = self.winfo_pointerx()
                y = self.winfo_pointery()
                mon_x, mon_y, mon_w, mon_h = get_monitor_at_point(x, y)
                
                # Center 1200x800 window on that monitor
                w, h = 1200, 800
                center_x = mon_x + (mon_w - w) // 2
                center_y = mon_y + (mon_h - h) // 2
                geometry = f"{w}x{h}+{center_x}+{center_y}"
            except Exception:
                geometry = "1200x800"
    ...
    def _build_ui(self) -> None:
        ...
        # Feed tab disabled for now - not creating it
        ...
        # Feed Tab Component disabled for now
    ...
    def _build_left_panel(self) -> None:
        ...
        # Header
        self._header = ctk.CTkFrame(self._left_panel, fg_color="transparent", height=50)
        ...
        # Status indicator
        self._status = StatusIndicator(self._header)
        self._status.pack(side="right", padx=10)
        
        # Player area
        self._player_frame = ctk.CTkFrame(self._left_panel, fg_color="#0d0d1a", corner_radius=10)
        self._player_frame.pack(fill="both", expand=True, pady=(0, 10))
        
        # Embedded player (if VLC available) or placeholder
        if is_vlc_available():
            vlc_path = self._player_manager.get_vlc_lib_path()
            self._player = EmbeddedPlayer(self._player_frame, vlc_lib_path=vlc_path)
    ...
    def _build_right_panel(self) -> None:
        ...
        self._refresh_btn = ctk.CTkButton(
            ...
            command=self._check_favorites_live
        )
    ...
    def _on_clipboard_url(self, url: str) -> None:
        ...
        # Schedule execution on main thread to avoid "main thread is not in main loop" error
        # This is called from ClipboardMonitor's background thread
        self.after(0, lambda: self._handle_clipboard_url(url))
    
    def _handle_clipboard_url(self, url: str) -> None:
        """Handle URL detected in clipboard (runs on main thread)."""
    ...
    def _enter_cinema_mode(self) -> None:
        """Enter cinema mode - fullscreen window with only video."""
        if self._cinema_mode:
            return
        ...
        try:
            ...
            # Hide all UI elements including header
            ...
            # Get current monitor dimensions to ensure we fullscreen on the CORRECT monitor
            from core.monitor_utils import get_monitor_at_point
            ...
            # Get bounds of the monitor containing the window center
            mon_x, mon_y, mon_w, mon_h = get_monitor_at_point(x, y)
            
            # Use overrideredirect (borderless) instead of native fullscreen
            # This is much safer for multi-monitor logic as we manually control positioning
            self.overrideredirect(True)
            
            # Explicitly set geometry to match the monitor bounds completely
            self.geometry(f"{mon_w}x{mon_h}+{mon_x}+{mon_y}")
            
            # Force update and lift to ensure it's visible on top
            self.update()
            self.lift()
            ...
            self._log(f"Error al entrar a Modo Cine: {e}", "ERROR")
            # Try to recover
            try:
                self._exit_cinema_mode()
            except:
    ...
    def _exit_cinema_mode(self) -> None:
        ...
        try:
            ...
            # Restore window borders FIRST
            self.overrideredirect(False)
            
            # Restore geometry if saved, otherwise default
            if hasattr(self, '_pre_cinema_geometry'):
                self.geometry(self._pre_cinema_geometry)
            
            self.update()
            # To put header back on top, we must unpack player_frame temporarily
            ...
            # 2. Player (always visible, expands)
            if hasattr(self, '_player_frame'):
                self._player_frame.pack(fill="both", expand=True, pady=(0, 10))
            ...
            self._log(f"Error al salir de Modo Cine: {e}", "ERROR")
    ...
    def _check_favorites_live(self) -> None:
        """Check all favorites for live status (threaded)."""
        if self._checking_favorites:
            return  # Already checking
        
        self._checking_favorites = True
        self._refresh_btn.configure(text="⏳ ...", state="disabled")
        
        # Get favorite URLs
        favorites = self._history_manager.get_favorites()
        urls = [f.url for f in favorites]
        
        if not urls:
            self._checking_favorites = False
            self._refresh_btn.configure(text="🔄 Revisar", state="normal")
            self._log("No hay favoritos para revisar")
            return
        
        def check_in_background():
            """Background thread for checking streams."""
            from core.stream_checker import StreamChecker
            
            checker = StreamChecker(self._stream_engine)
            
            def on_progress(url, status):
                """Update UI on main thread."""
                try:
                    self.after(0, lambda u=url, s=status: self._update_card_status(u, s))
                except Exception:
                    pass
            
            # Check all URLs
            checker.check_batch(urls, on_progress=on_progress)
    ...
    def _update_card_status(self, url: str, status: str) -> None:
        """Update card status on main thread."""
        if url in self._history_cards:
            self._history_cards[url].set_live_status(status)
