...
class DirectURLStrategy(ExtractionStrategy):
    ...
    def can_extract(self, html: str) -> bool:
        ...
        # Match .flv or .m3u8 followed by quote, or with query params
    ...
    def extract(self, html: str, cookies: dict = None) -> Optional[Dict]:
        ...
        try:
            ...
            def decode_url(url):
                # Replace common Unicode escapes in URLs
                url = url.replace('\\u0026', '&')
                url = url.replace('\\u003d', '=')
                url = url.replace('\\u003f', '?')
                return url
            
            flv_urls = [decode_url(url) for url in flv_urls]
            m3u8_urls = [decode_url(url) for url in m3u8_urls]
